...
void OpenPolyline::Add(const Position& node) {
  if (nodes_.size() < 1) {
    ...
    lines_.push_back(
      Line {
        Position { nodes_.at(nodes_.size() - 2) },
...
const Position& OpenPolyline::At(unsigned int index) const {
...
const Color& OpenPolyline::GetColor() const {
  return color_;
}
